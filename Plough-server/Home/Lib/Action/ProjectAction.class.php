...
class ProjectAction extends Action {
	...
	public function getAllSolProject() {
        ...
        $customFeild = M('project_custom_field')
                       ...
                       ->select(); 
        if($customFeild){
        	$fieldSql = 'concernPeople,id,'.$customFeild[0]['customField'];
        }
	 ...
	 *
	 */
	public function updateSolProject(){
		if (! empty ( $_FILES )) {
			import ( "ORG.Util.PHPExcel" );
			// 上传excl
			...
			foreach ($data as $value){
				$oneProject['projectId'] = $value['A'];
				$oneProject['projectName'] = $value['B'];
				$oneProject['customerName'] = $value['C'];
				$oneProject['projectPrice'] = $value['E'];
				$oneProject['isNew'] = '1';
				$selectData = $projects
				->where('projectId="'.$value['A'].'" and risk!=""')
				->select();
				...
				if($selectData){
					...
					if($result){
						$logs = M ( 'logs_info' );
						$logsData ['logDetail'] = date ( 'Y-m-d H:i:s' ) . ':' . session ( 'userName' ) . '导入更新项目'.$oneProject['projectId'];
						$logs->add ( $logsData );
					}
				}else{
					//risk 为空 isNew=1 为战计部同步项目
					$isWorkProject = $projects
					->where('projectId="'.$value['A'].'" and risk is null')
					->select();
					if($isWorkProject){
						$result = $projects
						->where('projectId="'.$value['A'].'" and risk is null')
						->save($oneProject);
						$logs = M ( 'logs_info' );
						$logsData ['logDetail'] = date ( 'Y-m-d H:i:s' ) . ':' . session ( 'userName' ) . '导入更新报工项目'.$oneProject['projectId'];
						$logs->add ( $logsData );
					}else{
						$oneProject['risk'] = '未知';
						$result = $projects->add($oneProject);
						if($result){
							$logs = M ( 'logs_info' );
							$logsData ['logDetail'] = date ( 'Y-m-d H:i:s' ) . ':' . session ( 'userName' ) . '导入添加项目'.$oneProject['projectId'];
							$logs->add ( $logsData );
						}
					}
				}
	 ...
	 * 根据项目名称更新项目编号
	 */
	public function updateSolProjectId() {
		if (! empty ( $_FILES )) {
			import ( "ORG.Util.PHPExcel" );
			// 上传excl
			$uploadPath = './Public/upload/userInfo/';
			...
			//更新项目编号
			foreach ($data as $value){
				$sql = 'update pmo_projects set projectId ="'.$value['A'].
				'" where projectName="'.$value['B'].'" and risk!=""';
				M('projects')->query($sql);
				$logs = M ( 'logs_info' );
				$logsData ['logDetail'] = date ( 'Y-m-d H:i:s' ) . ':' . session ( 'userName' ) . '导入执行:'.$sql;
				$logs->add ( $logsData );
