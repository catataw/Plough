  ...
  actions:{
    ...
    searchedItemFromWord:function () {
      ...
      loadUsers.forEach(function (user) {
        console.log(user);
        if(user['tenantName'].toString().toLowerCase().indexOf(searchWord)>=0||
          user['officerName'].toString().toLowerCase().indexOf(searchWord)>=0
        ){
    ...
    userEdit:function () {
      ...
      request({
        ...
        if(data){
          ...
          self.set('errorInfo','编辑失败，编辑租户结果异常');
        }
      })
    },
    ...
    doAddTeam:function () {
      ...
      request({
        ...
        if(data){
          ...
          self.set('errorInfo','添加失败，添加租户异常！');
        }
      })
    }
  }
